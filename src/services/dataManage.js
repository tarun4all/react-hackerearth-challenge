...
const dataFilter = (data) => {
    const brand = [], variety = [], style = [], country = [];
    data.forEach((element) => {
        !brand.includes(element.Brand) && element.Brand !== 'Nan' && brand.push(element.Brand);
        !variety.includes(element.Variety) && element.Variety !== 'Nan' && variety.push(element.Variety);
        !style.includes(element.Style) && element.Style !== 'Nan' && style.push(element.Style);
        !country.includes(element.Country) && element.Country !== 'Nan' && country.push(element.Country);
    });

    return {brand, variety, style, country};
}

const call = () => {
    // let res = fetch('http://starlord.hackerearth.com/TopRamen').then((response) => response.json())
    // .then((data) => {
    //   return data;
    // })
    return fakeData;
